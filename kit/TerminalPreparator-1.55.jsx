...
function showAllOkDanceDialog(reportText) {
    ...
    try {
        if (typeof $.screens !== "undefined" && $.screens && $.screens.length > 0) {
            var scrH2 = $.screens[0].bottom - $.screens[0].top;
            if (scrH2 > 0) artH = Math.min(440, Math.max(320, Math.floor(scrH2 * 0.48)));
        }
    } catch (eArt) {}
    ...
    et.preferredSize = [Math.max(boxW, artW), 72];
    et.alignment = ["fill", "top"];

    var danceWrap = w.add("group");
    danceWrap.alignment = ["center", "bottom"];
    var dancer = danceWrap.add("edittext", undefined, frames[0], {
        multiline: true,
        readonly: true,
        scrolling: false
    });
    dancer.preferredSize = [artW, artH];
